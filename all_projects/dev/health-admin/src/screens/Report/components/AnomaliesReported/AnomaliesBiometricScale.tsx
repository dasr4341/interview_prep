...
import { AnomaliesPatientList } from './AnomalyPatientList';

export default function AnomaliesBiometricScale({
  data,
}: {
  data: AnomaliesPatientList;
}) {
  ...
  return (
    <div className="flex flex-1 py-3">
      <div className="relative">
        <HeatmapScale />
        <HeatmapScaleEllipse
          className={`absolute top-0 -mt-1.5 ${getStyle(data.scale)} `}
        />
      </div>
    </div>
  );
