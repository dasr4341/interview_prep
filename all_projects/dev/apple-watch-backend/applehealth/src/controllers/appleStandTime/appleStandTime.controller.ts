...
import { AppleStandTimeModel, appleStandTimeModelGenerator } from "../../models/activities-model/appleStandTime.model.js";

export default class AppleStandTimeController {
  async appleStandTimeDate(req: Request | any, res: Response, next: NextFunction) {
    try {
      ...
      const response = await appleStandTimeModel.aggregate([
  ...
  async appleStandTimeDateRange(req: Request | any, res: Response, next: NextFunction) {
    try {
  ...
  async appleStandTimeIntraday(req: Request | any, res: Response, next: NextFunction) {
    try {
      if (!req?.params?.stDate && !req?.params?.enDate) {
        throw new Exception(
          messagesData.errorList.heart.improperFields,
