...
export default class AppleExerciseTimeController {
  async exerciseTimeDate(req: Request | any, res: Response, next: NextFunction) {
    try {
      ...
      const exerciseTimeModel = await appleExerciseTimeModelGenerator(userId);

      const response = await exerciseTimeModel.aggregate([
  ...
  async exerciseTimeDateRange(req: Request | any, res: Response, next: NextFunction) {
    try {
  ...
  async exerciseTimeIntraday(req: Request | any, res: Response, next: NextFunction) {
    try {
      if (!req?.params?.stDate && !req?.params?.enDate) {
        throw new Exception(
          messagesData.errorList.heart.improperFields,
