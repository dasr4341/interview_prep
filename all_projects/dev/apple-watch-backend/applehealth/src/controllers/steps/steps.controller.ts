...
export class StepsController {
  async stepsDate(req: Request | any, res: Response, next: NextFunction) {
    try {
      ...
      const response = await stepsModel.aggregate([
      ...
      return responseLib.success(res, StatusCodes.OK, 'steps data get success', response);
    } catch (error) {
      next(error);
    }
  }

  async stepsDateRange(req: Request | any, res: Response, next: NextFunction) {
    try {
      const { userId } = req.user;
      ...
      return responseLib.success(res, 201, 'steps date range get success', response);
    } catch (error) {
      return next(error);
    }
  }

  async stepsDateTimeRange(req: Request | any, res: Response, next: NextFunction) {
    try {
      const { userId } = req.user;
      ...
      const stepsModel = await stepsModelGenerator(userId);
      const stepsResponse = await stepsModel.aggregate([
        { $match: { time: { $gte: startUtcDate, $lte: endUtcDate } } },
        { $group: { _id: '$time', value: { $last: '$value' } } },
        { $sort: { _id: 1 } },
        {
          $project: {
            _id: 0,
      ...
      return responseLib.success(res, 201, 'steps date time range get success', stepsResponse);
    } catch (error) {
      return next(error);
    }
  }
