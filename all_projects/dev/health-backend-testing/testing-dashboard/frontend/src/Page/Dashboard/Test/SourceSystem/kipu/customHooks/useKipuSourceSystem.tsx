...
import { GetKipuDataPayload } from 'Lib/Api/Axios/sourceSystem/sourceSystemApi.interface';
...
function getStoredFacilitiesData() {
  const facilitiesFromLocalStorage = localStorage.getItem(
    config.storage.facilityList.kipuFacilityList
  );
...
export default function useKipuSourceSystem({
  onCompleted,
  onError,
}: {
  onCompleted: () => void;
  ...
  const getFacilities = async () => {
    try {
      ...
      localStorage.setItem(config.storage.facilityList.kipuFacilityList, JSON.stringify(list));
      onError(null);
    } catch (e: any) {
  ...
  async function getSourceSystemData(data: GetKipuDataPayload) {
    try {
      setLoading(true);
      const response = await sourceSystemApi.getKipuData(data);
