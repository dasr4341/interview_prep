...
const MarkAsSold = () => {
  ...
  return (
    <>
      <div className="bg-green-700 py-6 my-10  rounded-lg shadow-lg w-3/4 mx-auto">
        <div className="flex justify-center ">
          <div className="bg-white rounded-full p-6 shadow-md mb-4">
            <GiCheckMark className=" text-green-700" size={40} />
          </div>
        </div>

        <div className="text-center mb-4 text-white font-bold">
          <h2 className="text-2xl">Is the car sold ?</h2>
      ...
      <ApproveModal
        ...
        onConfirm={() => {
          updateCarStatus(carId, CarStatus.Sold);
          setIsModalOpen(!isModalOpen);
        }}
        open={isModalOpen}
        title="Are you sure you want mark car as sold ?"
      />
    </>
  );
};
