...
import { GMM_Anomaly, Monthly_Report_Data, Stats, Tanaka_Anomaly, Weekly_Monthly_Fillup_Data } from '../../interface/report_interface.js';
...
export async function create_monthly_report(
  id: string,
  ...
  try {
    console.log(`create_monthly_report - ${id}: start`);
    ...
    */
    ...
    setting api time
    */
    console.log(`create_monthly_report - ${id}: setting api time`);
    const _zoned_time = await zoned_time(timezone);
    const report_utc_time = new Date();
    console.log(`create_monthly_report - ${id}: zoned_time`, _zoned_time);

    // let end_api_date_object: Date;
    // if (isLastDayOfMonth(_zoned_time)) {
    //   end_api_date_object = _zoned_time;
    // } else {
    //   end_api_date_object = zonedTimeToUtc(subDays(startOfMonth(_zoned_time), 1), 'UTC');
    // }
    // const start_api_date_object = zonedTimeToUtc(startOfMonth(end_api_date_object), 'UTC');
    ...
    */
    console.log(`create_monthly_report - ${id}: date_range: start: ${start_api_date} end: ${end_api_date}`);
    ...
    */
    if (!response) {
      console.log(`create_monthly_report - ${id}: response: ${response}, not generating monthly report`);
      return;
    }
    ...
    get json files
    */
    console.log(`create_monthly_report - ${id}: fetching json files from: ${config.aws.s3.data_bucket}`);
    let monthly_data: Weekly_Monthly_Fillup_Data[] = [];
    ...
    monthly_data = bucket_data_response.map((e) => {
      if (e.status === 'fulfilled') return e.value;
      return null;
    });

    /*
    filter monthly_data for falsey values
    */
    monthly_data = monthly_data.filter((e) => e);

    /*
    if monthly_data < 2, return
    */
    if (monthly_data.length < 2) {
      console.log(`create_monthly_report - ${id}: monthly_data < 2, not generating monthly report, monthly_data: ${monthly_data}`);
      return;
    }
    ...
    */
    const monthly_data_date_array = monthly_data.map((e) => format(new Date(e.eventDate), 'yyyy-MM-dd'));
    console.log(`create_monthly_report - ${id}: monthly_data_date_array`, monthly_data_date_array);
    ...
    console.log(`create_monthly_report - ${id}: fill_up_date_array`, fill_up_date_array);

    for (const fill_up_date of fill_up_date_array) {
      if (monthly_data_date_array.includes(fill_up_date)) {
        console.log(`create_monthly_report: ${fill_up_date} data already exists`);
        const index = monthly_data.findIndex((e) => format(new Date(e.endApiDate), 'yyyy-MM-dd') === fill_up_date);
        if (monthly_data[index]?.heart?.reuse_heartCurrentDay) {
          monthly_data[index].heart.heartCurrentDay = structuredClone(monthly_data[index].heart.reuse_heartCurrentDay) as Heart[];
          monthly_data[index].steps.stepsCurrentDay = structuredClone(monthly_data[index].steps.reuse_stepsCurrentDay) as Steps[];
          monthly_data[index].heartStepsAnomaly = structuredClone(monthly_data[index].reuse_heartStepsAnomaly) as GMM_Anomaly[] | null;
          monthly_data[index].tanakaAnomaly = structuredClone(monthly_data[index].reuse_tanakaAnomaly) as Tanaka_Anomaly[] | null;
        }
      } else {
        console.log(`create_monthly_report - ${id}: ${fill_up_date} filling up data`);
    ...
    */
    for (let i = 0; i < monthly_data.length; i++) {
      const startApiDate = formatInTimeZone(new Date(monthly_data[i].startApiDate), 'UTC', 'yyyy-MM-dd');
      const endApiDate = formatInTimeZone(new Date(monthly_data[i].endApiDate), 'UTC', 'yyyy-MM-dd');
      console.log(`create_monthly_report - ${id}: modifying label, startApiDate :${startApiDate} endApiDate: ${endApiDate}`);

      let heart = monthly_data[i].heart.heartCurrentDay;
      let steps = monthly_data[i].steps.stepsCurrentDay;
      ...
      monthly_data[i].heart.heartCurrentDay = heart;
      monthly_data[i].steps.stepsCurrentDay = steps;
    }

    /*
    compute heart, steps, anomaly data
    */
    console.log(`create_monthly_report - ${id}: computing heart, steps, anomaly data`);
    ...
    for (let i = 0; i < monthly_data.length; i++) {
      /*
      updating monthly_data for heart and steps with isAnomaly key
      */

      /*
      computing anomaly data
      */
      if (monthly_data[i]?.heartStepsAnomaly || monthly_data[i]?.tanakaAnomaly) {
        const gmm_anomaly_arr = monthly_data[i].heartStepsAnomaly ?? [];
        const tanaka_anomaly_arr = monthly_data[i].tanakaAnomaly ?? [];

        const heart = monthly_data[i].heart.heartCurrentDay.map((e) => {
        ...
        const steps = monthly_data[i].steps.stepsCurrentDay.map((e) => {
          const match_gmm_index = gmm_anomaly_arr.findIndex((o) => e.time.includes(o.time));
          const match_tanaka_tndex = tanaka_anomaly_arr.findIndex((o) => e.time.includes(o.time));
          return {
            time: e.time,
      ...
      */
      const heart = monthly_data?.[i]?.heart?.heartCurrentDay;
      const steps = monthly_data?.[i]?.steps?.stepsCurrentDay;
      ...
      if (heart?.length || steps?.length) {
        ...
        for (let j = 0; j < length; j += chunk_size) {
          ...
          if (!heart_filtered.length) {
            ...
            if (monthly_data[i].heartStepsAnomaly) {
            ...
            if (monthly_data[i].tanakaAnomaly) {
              const anomaly_count = heart_filtered.reduce((acc, curr) => {
    ...
    */
    console.log(`create_monthly_report - ${id}: computing data to make monthly data object`);
    ...
    */
    ...
    if (source_system === Source_System.APPLEWATCH) {
      console.log(`create_monthly_report - ${id}: checking for temperature support`);
    ...
    */
    ...
    /*
    creating monthly data object
    */
    console.log(`create_monthly_report - ${id}: creating monthly report data object`);
    const data: Monthly_Report_Data = {
      heart: {
      ...
      dataURI: undefined,
      reportURI: undefined,
      reportType: config.report.monthly_report.type,
      reportName: config.report.monthly_report.name,
    ...
    /*
    check if monthly dataset is empty
    */
    console.log(`create_monthly_report - ${id}: checking if monthly report data is valid/empty`);
    ...
    console.log(`create_monthly_report - ${id}:`, { heart_empty, steps_empty, spo2_empty, sleep_empty, hrv_empty, temp_empty });
    if (heart_empty && steps_empty && spo2_empty && sleep_empty && hrv_empty && temp_empty) {
      console.log(`create_monthly_report - ${id}: dataset empty or invalid, not generating report`);
      return;
    }
    ...
    create data buffer
    */
    console.log(`create_monthly_report - ${id}: creating data buffer`);
    const data_buffer = report_helper.create_buffer(data);

    /*
    create pdf buffer
    */
    console.log(`create_monthly_report - ${id}: creating pdf buffer`);
    const pdf_buffer = await create_pdf(data, 'report.html');
    if (!pdf_buffer) {
      throw new Error('create_monthly_report: pdf_buffer not available');
    }

    /*
    screenshot
    */
    console.log(`create_monthly_report - ${id}: creating screenshot`);
    const sc = await screenshot(data, 'screenshot.html');
    if (!sc) {
      throw new Error('create_monthly_report: screenshot not available');
    }

    /*
    upload data
    */
    console.log(`create_monthly_report - ${id}: uploading data`);
    ...
    upload report
    */
    console.log(`create_monthly_report - ${id}: uploading report`);
    const report_uri = await upload_report(
      ...
      data.reportId,
      data.dataId,
      config.report.monthly_report.sqs_name,
      config.aws.sqs.set_url,
      `:${config.report.monthly_report.tag_name}`,
      '',
      null,
      null,
      sc,
    ...
    if (config.dev_mode) {
      console.log(`create_monthly_report - ${id}: creating local json data`);
    ...
    */
    ...
    console.log(`create_monthly_report - ${id}: error`, error);
    await notify(`create_monthly_report - ${id}: error`, error);
  } finally {
    console.log(`create_monthly_report - ${id}: end`);
  }
}
