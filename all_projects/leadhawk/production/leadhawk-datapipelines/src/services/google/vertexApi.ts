...
export async function getCompanyLinkedInURL(
  ...
  try {
    ...
    const vertexData = JSON.parse(getUTF8Data(responseText)) as VertexResponse;
    ...
    await GoogleSearchModel.updateOne(
      { query: query },
      {
        $set: {
          query,
          ...{ linkedInUrl, scrapeCompanyName: googleCompanyName },
          results: vertexData.results,
        },
      },
      { upsert: true }
    );
    ...
    if (error instanceof AxiosError) {
      ...
      logger.error(" :: Error making search request: :: ", { error });
    }
    return null;
  }
}
