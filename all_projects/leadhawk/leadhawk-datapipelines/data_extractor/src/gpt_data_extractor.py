...
gpt_model          = os.getenv(key = 'OPENAI_MODEL_NAME', default = 'gpt-3.5-turbo')
model_temperature  = float(os.getenv(key = 'GPT_MODEL_TEMPERATURE', default = 0.0))
model_seed         = int(os.getenv(key = 'GPT_SEED', default = 42))
max_response_token = int(os.getenv(key = 'GPT_MAX_TOKENS', default = 512))
max_retries        = int(os.getenv(key = 'GPT_MAX_RETRIES', default = 5))
base_delay         = int(os.getenv(key = 'BASE_DELAY', default = 0))
    ...
    try:
        ...
        for attempt in range(max_retries):
            try:
                ...
                raw_gpt_response = await openai_client_instance.completions.create(model       = gpt_model,
